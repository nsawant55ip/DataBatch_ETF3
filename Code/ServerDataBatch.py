...
"C:\Python3\python" "C:\DataBatch_ETF_NewProject\Code\ServerDataBatch.py" -A -R5 -T
"""
...
def processOptions():
    ...
    parser.add_argument("-C", "--Config", dest="Config",
                        default="DataBatchConfig.yml", help="Configuration file")
    parser.add_argument("-R", "--RunMode", dest="RunMode",
                        default='TEST', help="'TEST' mode won't touch the CompleteFlag or Archive.")
    ...
    parser.add_argument("-O", "--Output", action="store_false", dest="Output",
                        default=True, help="Supply this argument you do not want the output dir to be cleared before commence.")
...
def main():
    args = processOptions()
    config_file = os.path.join(BASEpath, 'Config', args.Config)
    ...
    try:
        ...
        if len(Unaccessible) > 0:
            ...
            DBU.SendRobustEmail(TeamEmails, # Recipients
                                [], # Attachments
                                message, # Text
                                'Network Drive Issue on ' + os.environ.get('COMPUTERNAME', 'UNKNOWN')) # Subject
        else:
            logfile.write(' ' + DBU.TimeString() + ' All network drive locations successfully accessed.\n')
        logfile.write('\n')

        # Clear out the output directory if argument to not clear the outfiles is not provided
        if not args.Output:
        ...
        while True:
            ...
            for Process in [elem for elem in list(ProcStatus.keys()) if ProcStatus[elem] == 'launched' and ProcObjects[elem].poll() != None]:
                (WriteFlag, ArchiveStack, HardDriveActive, EarliestStarts, ProcStatus) = DBU.ReceiveProcess(
                        Process, ProcObjects[Process], config, logfile, WriteFlag, ArchiveStack,
                        HardDriveActive, ProcStatus, EarliestStarts, TeamEmails, FLAGdir
                    )
            ...
            for Process in [elem for elem in list(ProcStatus.keys()) if ProcStatus[elem] == 'launched' and ProcObjects[elem].poll() == None and config[elem]['method'] == 'MQA']:
                (ProcessStatus, ProcessObjects, config, WriteFlag) = DBU.TimeOutMQA(Process, ProcStatus, ProcObjects, config, logfile, WriteFlag)
            ...
            if len([elem for elem in list(ProcStatus.keys()) if ProcStatus[elem] == 'launched' and config[elem]['method'] == 'MQA' and config[elem]['wait']]) == 0:
                for Process in [elem for elem in list(ProcStatus.keys()) if ProcStatus[elem] == 'unlaunched']:
                    ...
                    if DBU.CanLaunch(Process, ProcStatus, config, EarliestStarts, HardDriveActive):
                        (config, ProcObjects, ProcStatus, EarliestStarts) = DBU.LaunchProcess(
                                Process, config, logfile, ProcObjects, ProcStatus, EarliestStarts)
